...
export default async function Roster() {
  ...
  if (!s) redirect("/login");
  await migrate();
  const showAll = isSuper(s);
  const players = await query<any>(showAll
    ? "SELECT * FROM players ORDER BY active DESC, skill DESC"
    : "SELECT * FROM players WHERE active ORDER BY skill DESC");
  const users = isSuper(s) ? await query<any>("SELECT id,username,role FROM users ORDER BY username") : [];
  return (
    <>
      <h1 className="page">Roster</h1>
      <p className="sub">
        {players.filter((p: any) => p.active).length} players. Skill = 40% ball control + 35% influence + 25% discipline.
      </p>
